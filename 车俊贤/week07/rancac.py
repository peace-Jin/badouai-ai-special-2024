...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    best_inlier_idxs = None  # 最优解序号
    while iterations < k:  # 迭代次数小于k的时候:
...
def test():
    # 生成理想数据 1.
    n_samples = 500  # 样本个数 2.
    n_inputs = 1  # 输入变量个数，指的是每个样本数据只有一个自变量
    n_outputs = 1  # 输出变量个数
    A_exact = 20 * np.random.random((n_samples, n_inputs))
    # 随机生成0-20之间的500个数据只有一个自变量的数据矩阵:行向量，500行1列
    perfect_fit = 60 * np.random.normal(size=(n_inputs, n_outputs))
    # 随机线性度，即随机生成一个斜率,np.random.normal() 用来生成一个正态分布的随机数，
    # 默认的正态分布的均值为 0，标准差为 1。
    # n_inputs = 1 和 n_outputs = 1，因此 perfect_fit 将是一个形状为 (1, 1) 的矩阵，也就是一个单一的值。
    # 乘以 60。目的是放大生成的随机斜率，使得斜率在 60 倍的尺度上变化。
    # 乘以 60 是为了增加模型的随机性，确保生成的模型具有较大的波动性，可能使得拟合模型不那么简单、更加复杂。
    ...
    # 在这段代码中，每个值都会加上一个来自高斯分布的 随机数，从而达到 “模糊” 或 “扰动” 数据的效果。
    # 加入 高斯噪声 的目的通常是为了模拟 真实世界中的不确定性 和 噪声干扰，
    # 从而让模型的训练过程更加具有挑战性和真实性。
    ...
    # run RANSAC 算法                    data    model  n     k    t    d
    ransac_fit, ransac_data = ransac(all_data, model, 50, 1000, 7e3, 300, debug=debug, return_all=True)
    #                              样本点 假设模型 最少样本点 最大迭代次数 阈值 阈值
    #         model - 假设模型:事先自己确定
    #         n - 生成模型所需的最少样本点
    #         k - 最大迭代次数
    #         t - 阈值:作为判断点满足模型的条件
